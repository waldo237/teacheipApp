<template>
  <!-- members starts -->
  <div>
    <v-layout row wrap class="mx-1 px-3 mb-0 pb-0" justify-center>
      <v-card-title
        class="title my-0 font-weight-light main-title"
      >Miembros de nuestra comunidad</v-card-title>
    </v-layout>
    <v-layout row wrap class="mx-5 px-3" justify-center>
      <v-text-field
        light
        class="mx-4 mt-0"
    ...
    <v-layout row wrap class="mx-1 px-3" justify-center>
      <v-layout class="memberCarousel mx-5 px-2">
          <v-card v-for="item in displayMembers()" :key="item._id" class="ma-2 pa-2 grey darken-4" width="110px">
        <router-link
          class="white--text"
          :to="'/member/'+ item._id"
        >
            <v-layout justify-center>
              <v-avatar v-if="item.photoURL">
                <v-img :src="item.photoURL" :lazy-src="item.photoURL" />
              </v-avatar>
              <v-avatar
                v-else
            ...
            <br />
            <v-layout class="mx-2">{{item.firstName}} {{item.lastName}}</v-layout>
        </router-link>
          </v-card>
      </v-layout>
    </v-layout>
